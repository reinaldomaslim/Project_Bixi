...
class TfBroadcaster(object):
    ...
    x_c, y_c, angular_z =0, 0, 0

    def __init__(self, nodename):
        rospy.init_node('tf_broadcaster')
        listener = tf.TransformListener()
        rospy.Subscriber("/localization", Twist, self.encoderCallback, queue_size = 50)
        ...
        while not rospy.is_shutdown():
            ...
            #laser tf
            br.sendTransform((0.284, 0.171, 0),
            ...
            br.sendTransform((self.x_c, self.y_c, 0),
                             tf.transformations.quaternion_from_euler(0, 0, self.angular_z*math.pi/180),
                             rospy.Time.now(),
                             "base_link",
                             "odom")
    ...
    def encoderCallback(self, msg):
        ...
        #for subsequent, modify the reading to compensate
            ...
            self.angular_z=msg.angular.z-self.yaw_off
            ...
            yaw_c=math.atan2(math.sin(self.angular_z*math.pi/180), math.cos(self.angular_z*math.pi/180))

            self.x_c=-linear_y+del_x*math.cos(yaw_c)-del_y*math.sin(yaw_c)-del_x
            self.y_c=linear_x+del_x*math.sin(yaw_c)+del_y*math.cos(yaw_c)-del_y
            ...
            # odom.pose.pose.position.x=self.x_c
            # odom.pose.pose.position.y=self.y_c
            # q=Quaternion()
            # q.x, q.y, q.z, q.w=tf.transformations.quaternion_from_euler(0, 0, self.angular_z*math.pi/180)
            # odom.pose.pose.orientation=q
            # self.odom_pub.publish(odom)
